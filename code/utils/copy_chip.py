...
class CopyChip():
    ...
    def WriteInExistingChip(self, chip_to_write_in):
        """
        Copies chip in already existing chip
        """
        chip_to_write_in.clear()
        ...
        for net, wire in self.chip.solution.items():
            ...
            point1 = net.target[0]
            point2 = net.target[1]
            dummy_gate1 = chip_to_write_in.getGridPoint(point1.x, point1.y, point1.z)
            dummy_gate2 = chip_to_write_in.getGridPoint(point2.x, point2.y, point2.z)
            dummy_net = Net(dummy_gate1, dummy_gate2)
            chip_to_write_in.netlist.append(dummy_net)
            ...
            chip_to_write_in.solution[dummy_net] = new_wire

            for point, neighbour in zip(wire.path, wire.path[1:]):
                dummy_point = chip_to_write_in.getGridPoint(point.x, point.y, point.z)
                dummy_neighbour = chip_to_write_in.getGridPoint(neighbour.x, neighbour.y, neighbour.z)
